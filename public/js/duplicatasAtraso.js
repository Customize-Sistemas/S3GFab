...
async function getAllDuplicatas(event){
  ...
  try {
    const requestDupAberto = await fetch(`/api/finances/dupAtraso?estab=${ESTAB.value}&cliIni=${clienteInicial}&cliFin=${clienteFinal}&dataIni=${DATAINICIAL.value}&dataFini=${DATAFINAL.value}`)
          ...
          <tr>
            <td>
              <button
                ...
                data-bs-html="true"
                title="<b>Nº Banco: ${dap.DUP_NUMER_BANCO === undefined ? '' : dap.DUP_NUMER_BANCO}</b>"
              >${dap.DUPLIC}</button>
            </td>
